...
def select(request):
    message='asdfasdf'
    context = ''
    return render(request, 'parsed_data/slect.html', context)
...
def first(request):
    ...
    template=loader.get_template('parsed_data/book.html')
    books = BookData.objects.all() #Bookdata에 있는 모든 객체를 불러옵니다
    context = {'books': books}
